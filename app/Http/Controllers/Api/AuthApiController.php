...
{
    use ValidationTrait, ProfileTrait, RazorpayTrait;
    public $platform = 'app';
    ...
    public function login(Request $request)
    {
        ...
        try {
            $values = $request->only('isdCode', 'phone', 'password', 'isRegistered', "userType");

            if ($values['isRegistered'] == false) {
                $user = new User();
                $user->isdCode = $values['isdCode'];
                $user->phone = $values['phone'];
                $user->userType = config('constants.userType')['customers'];
                $user->password = Hash::make('123456');
                if ($user->save()) {
                    // if ($this->createCustomer(['phone' => $values['isdCode'] . $values['phone']]) == false) {
                    //     return response()->json(['status' => 0, 'msg' => "Opps there might be some surver isuue happen, pleas retry again", 'payload' => (object)[]], config('constants.ok'));
                    // } else {
                    goto next;
                    // }
                } else {
                    return response()->json(['status' => 1, 'msg' => __('messages.serverErrMsg'), "payload" => ['tokenType' => 'Bearer', 'token' => $token, 'user' => $data]], config('constants.ok'));
                }
            } else {
                $user = User::where([
            ...
            next:
            $credentials = ['isdCode' => $values['isdCode'], 'phone' => $values['phone'], 'password' => $values['password'], "userType" => $values['userType']];
            ...
            if (Auth::attempt($credentials)) {
                ...
                if ($token) {
                    ...
                    if ($data === false) {
                        ...
                        return response()->json(['status' => 1, 'msg' => __('messages.loginSuccess'), "payload" => ['tokenType' => 'Bearer', 'token' => $token, 'user' => $data]], config('constants.ok'));
                    }
                } else {
                    return response()->json(['status' => 0, 'msg' => __('messages.serverErrMsg'), 'payload' => (object)[]], config('constants.ok'));
                }
